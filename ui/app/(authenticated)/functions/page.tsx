...
const EXAMPLES: Record<string, string> = {
    ...
    Rust: `struct Component;
impl Guest for Component {
    fn handler(req: Request) -> String {
        format!("Hello via Rust!")
    }
}`,
    Go: `package main
...
func (f FunctionImpl) Handle(input string) string {
	return fmt.Sprintf("Hello from Go Component! You sent: %s", input)
}

func main() {
	fluor.SetFunction(FunctionImpl{})
}`
};
...
export default function FunctionsPage() {
    ...
    const executeInvoke = async () => {
        if (!selectedFunction || !selectedRoute) {
            setInvokeResult('Error: No route selected. Please create a trigger for this function.');
            return;
        }
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-[var(--accents-5)]">Manage your serverless functions.</p>
                </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="border-b border-white/20 dark:border-white/10 bg-black/5 dark:bg-white/5">
                        <tr>
                            ...
                            <th className="px-6 py-3 font-medium text-[var(--accents-5)]">Language</th>
                            <th className="px-6 py-3 font-medium text-[var(--accents-5)]">Executable</th>
                            <th className="px-6 py-3 font-medium text-[var(--accents-5)]">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/20 dark:divide-white/10">
                        {functions.map((func) => (
                            <tr key={func.name} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                                <td className="px-6 py-4 font-medium">{func.name}</td>
                                <td className="px-6 py-4 text-[var(--accents-5)]">{func.language}</td>
                                <td className="px-6 py-4 font-mono text-xs text-[var(--accents-5)]">{func.executable}</td>
                ...
                >
                    <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <label htmlFor="file-upload" className="text-sm font-medium text-[var(--accents-5)]">
                                Wasm Binary
                            </label>
                            <Input
                                id="file-upload"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Input
                                ...
                                onChange={(e) => setSelectedFunction({ ...selectedFunction, cpu: e.target.value })}
                            />
                            <Input
                                id="memory"
                                label="Memory"
                                value={selectedFunction.memory}
                                onChange={(e) => setSelectedFunction({ ...selectedFunction, memory: e.target.value })}
                            />
                        </div>
                    ...
                    <div className="space-y-4">
                        {availableRoutes.length > 0 ? (
                            <Select
                                ...
                                options={availableRoutes.map(r => ({ value: r.name, label: `${r.method} ${r.path} (${r.name})` }))}
                            />
                        ) : (
                            <div className="rounded-md bg-yellow-50 dark:bg-yellow-900/20 p-3">
                                <p className="text-sm text-yellow-700 dark:text-yellow-400">
                                    No triggers found for this function. Create a trigger to invoke it.
                                </p>
                            </div>
                        )}

                        <div className="space-y-1.5">
                            <label htmlFor="input" className="text-sm font-medium text-[var(--accents-5)]">Input</label>
                            <textarea
                                id="input"
                                value={invokeInput}
                            ...
                            />
                            {selectedRoute && availableRoutes.find(r => r.name === selectedRoute)?.method === 'GET' && (
                                <p className="text-xs text-[var(--accents-5)]">Input body is ignored for GET requests.</p>
                            )}
                        </div>

                        {invokeResult && (
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium text-[var(--accents-5)]">Result</label>
                        ...
                        <div className="flex justify-end gap-2 mt-6">
                            ...
                            <PrimaryButton onClick={executeInvoke}>
                                Run
                            </PrimaryButton>
                        </div>
                    </div>
